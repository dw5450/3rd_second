...
int main(int argc, char *argv[])
{
	...
	while(1){
		...
		// 접속한 클라이언트 정보 
		ConInfoStr += std::string("[TCP 서버] 클라이언트 접속: IP 주소=");
		ConInfoStr += inet_ntoa(clientaddr.sin_addr);
		ConInfoStr += std::string(", 포트 번호 = ");
		ConInfoStr += std::to_string(ntohs(clientaddr.sin_port));
		ConInfoStr += "\n";
